            ...
            for(let i = 0; i < 10; i++){
                ...
                if(data[i] == undefined){
                    ...
                    await  pool.request()
                      ...
                      .input("abbreviation", sql.VarChar, data[i].abbreviation)
                      .input("alternative_name", sql.VarChar, data[i].alternative_name)
                      .input("platofrm_enum", sql.Int, data[i].category)                      
                      .input("generation", sql.VarChar, data[i].generation.toString())                      
                      .input("name", sql.VarChar, data[i].name)                      
                      .input("platform_family", sql.Int, data[i].platform_family)                      
                      .input("summary", sql.VarChar, data[i].summary)                      
                      .input("url", sql.VarChar, data[i].url)                      
                      .query(querys.addplatoform)        
